...
var PeerConnectionClient = function(params, startTime) {
  ...
  this.groupId_ = params.groupId;

  // TODO(jiayl): Replace callbacks with events.
  // Public callbacks. Keep it sorted.
  this.onerror = null;
  this.iceConfigPromiseHandler = null;
  this.onIceServerConfigReady = null;
...
PeerConnectionClient.prototype.receiveSignalingMessage = function(message) {
  ...
  if (messageObj.type === SSConstants.SERVER) {
    ...
    this.onChannelReady(messageObj.roomId, messageObj.iceServers);
  } else if ((this.isInitiator_ && messageObj.type === SSConstants.ANSWER) ||
      (!this.isInitiator_ && messageObj.type === SSConstants.OFFER)) {

        trace('INFO: get sdp offer from remote');
    this.hasRemoteSdp_ = true;
    // Always process offer before candidates.
    this.messageQueue_.unshift(messageObj);
  } else if (messageObj.type === SSConstants.CANDIDATE) {
    this.messageQueue_.push(messageObj);
  } else if (messageObj.type === 'bye' || messageObj.type === SSConstants.HANG_UP) {
    if (this.onremotehangup) {
      this.onremotehangup();
    }
  } else if (messageObj.type === SSConstants.USER_DROPPED) {
    var result = confirm("The other user might dropped, do you want to close the call?");
    if (result) {
      if (this.onremotehangup) {
        this.onremotehangup();
      }
    ...
    trace('ERROR: uknown messag type.');
  }

  trace('INFO: call drainMessageQueue_:  hasRemoteSdp_ = ' + this.hasRemoteSdp_);
  this.drainMessageQueue_();
};

PeerConnectionClient.prototype.creatPeerConnection = function(iceSever) {
  ...
  '\'.');
    ...
    iceSever.Turn.forEach(element => {
      var ice = {
        credential: element.Credential,
        urls: element.Urls,
        username: element.Username
      }
      this.params_.peerConnectionConfig.iceServers.push(ice);
    });
    iceSever.Stun.forEach(element => {
      var ice = {
        credential: 'noneed',
        urls: element.Urls,
        username: 'noneed'
  ...
  this.pc_ = new RTCPeerConnection(
      this.params_.peerConnectionConfig, this.params_.peerConnectionConstraints);
...
PeerConnectionClient.prototype.onChannelReady = function(givenRoomId, ice) {
  /* Hank: cannot do this now, otherwise media does not flow from web to androd
  if(this.roomId_ !== null && this.roomId_ !== "" && this.roomId_ === givenRoomId) {
    this.isInitiator_ = true;
    this.startAsCaller({});
  }
  */
  this.creatPeerConnection(ice);
  this.roomId_ = givenRoomId;
  if(this.onIceServerConfigReady && this.iceConfigPromiseHandler){
    this.onIceServerConfigReady(this.iceConfigPromiseHandler);
  }
}
...
PeerConnectionClient.prototype.drainMessageQueue_ = function() {
  ...
  trace("this.pc_ = "+ this.pc_);
  trace("this.started_ = "+ this.started_);
  trace("this.hasRemoteSdp_ = "+ this.hasRemoteSdp_);
  if (!this.pc_ || !this.started_ || !this.hasRemoteSdp_) {
    trace('WARNING: return without process messages: in drainMessageQueue_ ');
    return;
  }
